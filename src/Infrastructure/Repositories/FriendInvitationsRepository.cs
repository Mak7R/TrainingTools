...
public class FriendInvitationsRepository : IRepository<FriendInvitation, (Guid InvitorId, Guid InvitedId)>
{
    private static readonly
        ReadOnlyDictionary<string, Func<string, Expression<Func<FriendInvitationEntity, bool>>>>
        FriendInvitationFilters = new(
            new Dictionary<string, Func<string, Expression<Func<FriendInvitationEntity, bool>>>>
            {
                {
                    FilterOptionNames.Relationships.FriendInvitation.Invited,
                    value => Guid.TryParse(value, out var invitedId) ? e => e.InvitedId == invitedId : _ => false
                },
                {
                    FilterOptionNames.Relationships.FriendInvitation.Invitor,
                    value => Guid.TryParse(value, out var invitorId) ? e => e.InvitorId == invitorId : _ => false
                }
            });

    private static readonly
        ReadOnlyDictionary<OrderModel, Func<IQueryable<FriendInvitationEntity>, IQueryable<FriendInvitationEntity>>>
        FriendInvitationOrders =
            new(new Dictionary<OrderModel, Func<IQueryable<FriendInvitationEntity>, IQueryable<FriendInvitationEntity>>>
            {
                {
                    new OrderModel
                    {
                        OrderBy = OrderOptionNames.Relationships.FriendInvitation.InvitationDateTime,
                        OrderOption = OrderOptionNames.Shared.Ascending
                    },
                    query => query.OrderBy(f => f.InvitationDateTime)
                },
                {
                    new OrderModel
                    {
                    ...
                    query => query.OrderByDescending(f => f.InvitationDateTime)
                }
            });
    ...
    public FriendInvitationsRepository(ApplicationDbContext dbContext, ILogger<FriendInvitationsRepository> logger,
        IMapper mapper)
    {
    ...
    public async Task<IEnumerable<FriendInvitation>> GetAll(FilterModel? filterModel = null,
    ...
    public async Task<FriendInvitation?> GetById((Guid InvitorId, Guid InvitedId) id)
    {
        ...
        {
            _logger.LogError(e, "Exception was thrown while receiving friend invitations by id '{id}' from database",
                id);
            throw new DataBaseException("Error while receiving friend invitations from database", e);
        }
    }
    ...
    public async Task<int> Count(FilterModel? filterModel = null)
    {
        ...
        {
            _logger.LogError(e,
                "Exception was thrown while receiving friend invitations count by filter '{filter}' from database",
                filterModel);
    ...
    public async Task<OperationResult> Create(FriendInvitation invitation)
    {
        ...
        {
            _logger.LogError("Users '{invitor}' and '{target}' already was invited", invitation.Invitor.Id,
                invitation.Invited.Id);
            return DefaultOperationResult.FromException(alreadyExistsException); // 
        }
        catch (Exception e)
        {
            _logger.LogError(e,
                "Exception was thrown while adding new friend invitation (Invitor: '{invitor}' Target: {target}) to database",
                invitation.Invitor.Id, invitation.Invited.Id);
            return DefaultOperationResult.FromException(
                new DataBaseException("Error while adding new friend invitation", e));
        }
    ...
    public async Task<OperationResult> Update(FriendInvitation friendInvitation)
    {
        try
        {
            var invitationEntity = await _dbContext.FriendInvitations
                .FirstOrDefaultAsync(fi =>
                    fi.InvitorId == friendInvitation.Invitor.Id && fi.InvitedId == friendInvitation.Invited.Id);

            if (invitationEntity is null)
            {
                _logger.LogError("Invitation ({invitor}; {target}) was not found", friendInvitation.Invitor.Id,
                    friendInvitation.Invited.Id);
        ...
        {
            _logger.LogError(e,
                "Exception was thrown while updating friend invitation (Invitor:'{invitor}' Target:{target}) to database",
                friendInvitation.Invitor.Id, friendInvitation.Invited.Id);
    ...
    public async Task<OperationResult> Delete((Guid InvitorId, Guid InvitedId) id)
    {
        ...
        try
        {
            var invitation = await _dbContext.FriendInvitations.FirstOrDefaultAsync(fi =>
                fi.InvitorId == id.InvitorId && fi.InvitedId == id.InvitedId);
        ...
        {
            _logger.LogError(e, "Exception was thrown while removing invitation ({invitor}; {target}) from database",
                id.InvitorId, id.InvitedId);

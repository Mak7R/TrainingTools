...
public class AccountController : ApiController
{
    ...
    public AccountController(UserManager<ApplicationUser> userManager, IMapper mapper,
        IAuthTokenService<TokenGenerationInfo> tokenService, IStringLocalizer<Controllers.AccountController> localizer)
    {
    ...
    public async Task<IActionResult> Register(RegisterDto registerDto, [FromServices] IEmailSender emailSender)
    {
        ...
        if (existsUser is not null)
        {
            if (existsUser.EmailConfirmed)
                return Problem("User with this email already exists", statusCode: 400,
        ...
        if (result.Succeeded)
        {
            ...
            var confirmationLink =
                Url.Action("ConfirmEmail", "EmailConfirmation", new { userId = newUser.Id, token }, Request.Scheme) ??
                $"?userId{newUser.Id}&token={token}";
            await emailSender.SendEmailAsync(registerDto.Email, _localizer["ConfirmEmailTitle"],
                _localizer["ConfirmEmailText", confirmationLink]);

            return Ok(new AuthenticationResponse
                { UserName = newUser.UserName, Email = newUser.Email, Roles = [nameof(Role.User)] });
        }
    ...
    public async Task<IActionResult> Login(LoginDto loginDto)
    {
        ...
        if (user is null) return Problem("Invalid login or password", statusCode: 400);

        if (!user.EmailConfirmed)
            return Problem("Email is not confirmed", statusCode: StatusCodes.Status403Forbidden,
        ...
        return Problem("Invalid login or password", statusCode: 400);
    }
        ...
        [FromServices] IEmailSender emailSender)
    {
        ...
        if (await _userManager.CheckPasswordAsync(user, updateProfileDto.CurrentPassword!))
        {
            ...
            if (emailUpdated)
            {
                ...
                await emailSender.SendEmailAsync(updateProfileDto.Email!, _localizer["ConfirmEmailTitle"],
                    _localizer["ConfirmEmailText", confirmationLink]);
            }
        ...
        return StatusCode(StatusCodes.Status403Forbidden,
            new ProblemDetails { Detail = "Invalid password", Status = 403 });
    }
    ...
    {
        if (email == null) return Problem("Email is empty", statusCode: 400, title: "Email is empty");
    ...
    {
        if (userName == null) return Problem("Username is empty", statusCode: 400, title: "Username is empty");

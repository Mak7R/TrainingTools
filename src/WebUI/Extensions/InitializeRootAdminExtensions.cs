...
{
    public static async Task InitializeRootAdminFromConfiguration(this IServiceProvider serviceProvider,
        IConfiguration configuration)
    {
        ...
        if (existsRoot == null)
        {
            ...
            if (result.Succeeded)
                await userManager.AddToRolesAsync(rootAdmin,
                    new[] { nameof(Role.User), nameof(Role.Admin), nameof(Role.Root) });
            else
